...
[IMPORTANT]
====
If you want to enable cluster-wide link:../concepts/concept_solidfire_concepts_security.html#encryption-at-rest-software[software encryption at rest], you must create the cluster using the link:../api/reference_element_api_createcluster.html[CreateCluster] API method instead and change the enableSoftwareEncryptionAtRest parameter to `true`. Software encryption at rest is enabled by default for SolidFire eSDS clusters. Beginning with Element 12.5, you can enable software encryption at rest in the Create Cluster UI for non SolidFire eSDS clusters.
