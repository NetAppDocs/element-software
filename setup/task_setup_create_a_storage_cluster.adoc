...
IMPORTANT: If you want to enable cluster-wide xref:concept_solidfire_element_ug_encryption_at_rest.adoc[software encryption at rest], you must create the cluster using the xref:reference_element_api_createcluster.adoc[CreateCluster] API method instead and change the enableSoftwareEncryptionAtRest parameter to `true`. Software encryption at rest is enabled by default for SolidFire eSDS clusters. Hardware-based encryption at rest can be xref:task_system_manage_cluster_enable_and_disable_encryption_for_a_cluster.adoc[enabled and disabled] after cluster creation.
