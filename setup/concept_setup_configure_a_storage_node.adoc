...
* <<Configure the storage node using the per-node UI,Per-node UI>>: Use the per-node UI (https://<node_management_IP>:442) to configure node network settings.

* <<Configure the storage node using the TUI,TUI>>: Use the node terminal user interface (TUI) to configure the node.
...
== Configure the storage node using the TUI
