...
* <<Automate the upgrade with Ansible>> _NEW!_
...
== Automate the upgrade with Ansible

You can automate the upgrade workflow by using Ansible to perform a rolling upgrade of the entire cluster. To get started, navigate to the https://github.com/NetApp-Automation[NetApp Ansible repository^] on GitHub and download the nar_solidfire_sds_upgrade role and documentation.
...
a|
. See <<Upgrading H610S storage nodes to Element 12.3 (phase 2)>> and complete the process for each node.
. Reauthenticate if needed and select *PUT ​/upgrades/{upgradeId}*.
. Select *Try it out*.
. Enter the upgrade ID from the previous step in *upgradeId*.
...
. For SolidFire storage systems, go to the Element software https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab[download page^].
