...
* For nodes running Element 12.x, when there are no firmware updates to install, the process takes 20-30 minutes per node.
* For H610S nodes running Element 12.x, when there are BIOS and BMC firmware updates to install, the process takes 60-70 minutes per node.
* For H610S nodes running Element 11.x, when there are firmware updates to install, the firmware updates keep the node offline for a period of time. As a result, the process takes approximately 2 hours per node.
====
...
. Go to the Element software https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab[downloads page^].
