...
NOTE: Beginning with Element 12.5, NetApp HealthTools is no longer supported for Element software upgrades. If you are running Element 11.0 or 11.1, you must first link:https://review.docs.netapp.com/us-en/hci_2.20-doc-3926-pc-hci/docs/task_hcc_upgrade_element_software.html#upgrade-element-software-at-connected-sites-using-healthtools[upgrade to Element 12.3 using HealthTools^] and then upgrade to Element 12.5 using NetApp Hybrid Cloud Control.
...
IMPORTANT: You must upgrade to the latest management services bundle before upgrading your Element software to version 12.3.x. If you are updating your Element software to version 12.5, you need management services 2.14.60 or later to proceed.
...
* *Updated baseboard management controller (BMC) for H610S storage nodes*: You have upgraded the BMC version for your H610S nodes. See the link:https://docs.netapp.com/us-en/hci/docs/rn_H610S_BMC_3.84.07.html[release notes and upgrade instructions^].
...
+
https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab[Element software upgrade package (.tar.gz) for SolidFire storage clusters^]
...
[TIP]
====
The upgrade process time varies depending on the Element version and firmware updates.

* For nodes running Element 12.x, when there are no firmware updates to install, the process takes approximately 20-30 minutes per node.
* For H610S nodes running Element 12.x, when there are BIOS and BMC firmware updates to install, the process takes approximately 60-70 minutes per node.
* For H610S nodes running Element 11.x, the firmware updates keep the node offline for a period of time. As a result, the process takes approximately 2 hours per node. You must also https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/H_Series/NetApp_H610S_storage_node_power_off_and_on_procedure[perform a complete node shutdown and power disconnect^] for each H610S node.
...
a|
. See <<Upgrading H610S storage nodes to Element 12.3.x (phase 2)>> and complete the process for each node.
. Reauthenticate if needed and select *PUT ​/upgrades/{upgradeId}*.
. Select *Try it out*.
. Enter the upgrade ID from the previous step in *upgradeId*.
