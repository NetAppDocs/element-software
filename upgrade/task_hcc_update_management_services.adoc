...
Beginning with management services 2.20, you must accept and save the End User License Agreement (EULA) before using the NetApp Hybrid Cloud Control UI or API to upgrade management services:
...
.Steps
. Open the IP address of the management node in a web browser: `\https://<ManagementNodeIP>`
. Log in to NetApp Hybrid Cloud Control by providing the storage cluster administrator credentials.
. Select *Upgrade* near the top right of the interface.
