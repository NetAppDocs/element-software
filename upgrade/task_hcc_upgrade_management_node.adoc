...
[.lead]
You can upgrade your management node to management node 12.8 from version 12.3.x or later.
...
== Step 2: Upgrade a management node to Element 12.8

Choose one of the following upgrade options:

* <<Upgrade a management node to version 12.8 from version 12.3.x or later>>
...
You can perform an in-place upgrade of the management node from version 12.3.x or later to version 12.8 without needing to provision a new management node virtual machine.

NOTE: The Element 12.8 management node is an optional upgrade. It is not required for existing deployments.

.Before you begin

* The RAM of the management node VM is 24GB.
* The management node you are intending to upgrade is version 12.0 and uses IPv4 networking. The management node version 12.8 does not support IPv6.
+
...
* If you are updating your management node to version 12.8, you need management services 2.21.61 or later to proceed.
