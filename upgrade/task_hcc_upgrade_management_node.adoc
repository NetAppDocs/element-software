...
keywords: netapp, cluster, management node, mNode, upgrade, 10, 11.0, 11.1, 11.3, 11.5, 11.7, 12.0, 12.2, 12.3
---
...
* You have configured an additional network adapter (if required) using the instructions for link:https://docs.netapp.com/us-en/hci/docs/task_mnode_install_add_storage_NIC.html[configuring an additional storage NIC].
+
...
. Download the management node ISO for https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab[Element] software from the NetApp Support Site to the management node virtual machine.
+
...
IMPORTANT: If you had previously disabled SSH functionality on the management node, you need to link:https://docs.netapp.com/us-en/hci/docs/task_mnode_ssh_management.html[disable SSH again] on the recovered management node. SSH capability that provides link:https://docs.netapp.com/us-en/hci/docs/task_mnode_enable_remote_support_connections.html[NetApp Support remote support tunnel (RST) session access] is enabled on the management node by default.
...
. (For all NetApp SolidFire all-flash storage installations with NetApp Element Plug-in for vCenter Server) Update the vCenter Plug-in on the 12.3 management node by following the steps in the link:task_vcp_upgrade_plugin.html[Upgrade the Element Plug-in for vCenter Server] topic.
