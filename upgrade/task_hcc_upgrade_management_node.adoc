...
keywords: netapp, hci, on premise, cluster, management node, mNode, upgrade, 10, 11.0, 11.1, 11.3, 11.5, 11.7, 12.0, 12.2, 12.3
---
...
[.lead]
You can upgrade your management node to management node version 12.3 from version 11.0 or later.
...
<<Upgrade a management node to version 12.3 from 12.2>>
* If you are upgrading from management node 12.0:
<<Upgrade a management node to version 12.3 from 12.0>>
* If you are upgrading from management node 11.3, 11.5, 11.7, or 11.8:
<<Upgrade a management node to version 12.3 from 11.3 through 11.8>>
* If you are upgrading from management node 11.0 or 11.1:
<<Upgrade a management node to version 12.3 from 11.1 or 11.0>>
...
You can perform an in-place upgrade of the management node from version 12.2 to version 12.3 without needing to provision a new management node virtual machine.

NOTE: The Element 12.3 management node is an optional upgrade. It is not required for existing deployments.

.What you'll need

* The RAM of the management node VM is 24GB.
* The management node you are intending to upgrade is version 12.0 and uses IPv4 networking. The management node version 12.3 does not support IPv6.
+
...
* If you are updating your management node to version 12.3, you need management services 2.14.60 or later to proceed.
...
. Download the management node ISO for [.line-through]#https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab[NetApp HCI] or# https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab[Element] software from the NetApp Support Site to the management node virtual machine.
+
NOTE: The name of the ISO is similar to `solidfire-fdva-<Element release>-patchX-XX.X.X.XXXX.iso`

. Check the integrity of the download by running md5sum on the downloaded file and compare the output to what is available on NetApp Support Site for NetApp [.line-through]#HCI or# Element software, as in the following example:
+
...
You can perform an in-place upgrade of the management node from version 12.0 to version 12.3 without needing to provision a new management node virtual machine.
...
You can perform an in-place upgrade of the management node from version 11.3, 11.5, 11.7, or 11.8 to version 12.3 without needing to provision a new management node virtual machine.

NOTE: The Element 12.3 management node is an optional upgrade. It is not required for existing deployments.

.What you'll need

* The management node you are intending to upgrade is version 11.3, 11.5, 11.7, or 11.8 and uses IPv4 networking. The management node version 12.3 does not support IPv6.
+
TIP: To check the version of your management node, log in to your management node and view the Element version number in the login banner.
...
== Upgrade a management node to version 12.3 from 11.1 or 11.0
You can perform an in-place upgrade of the management node from 11.0 or 11.1 to version 12.3 without needing to provision a new management node virtual machine.
...
* The management node you are intending to upgrade is version 11.0 or 11.1 and uses IPv4 networking. The management node version 12.3 does not support IPv6.
+
TIP: To check the version of your management node, log in to your management node and view the Element version number in the login banner.
...
. On the 12.3 management node, run the `upgrade-mnode` script to retain previous configuration settings.
+
...
. (For all NetApp [.line-through]#HCI installations and# SolidFire all-flash storage installations with NetApp Element Plug-in for vCenter Server) Update the vCenter Plug-in on the 12.3 management node by following the steps in the link:task_vcp_upgrade_plugin.html[Upgrade the Element Plug-in for vCenter Server] topic.
...
[.line-through]#. If you have a NetApp HCI installation, locate the hardware tag for your compute node in vSphere:#
[.line-through]#.. Select the host in the vSphere Web Client navigator.#
[.line-through]#.. Click the *Monitor* tab, and click *Hardware Health*.#
[.line-through]#.. The node BIOS manufacturer and model number are listed. Copy and save the value for `tag` for use in a later step.#
[.line-through]#. Add a vCenter controller asset for HCI monitoring (NetApp HCI installations only) and Hybrid Cloud Control (for all installations) to the management node known assets:#
[.line-through]#.. Click *POST /assets/{asset_id}/controllers* to add a controller sub-asset.#
[.line-through]#.. Click *Try it out*.#
[.line-through]#.. Enter the parent base asset ID you copied to your clipboard in the *asset_id* field.#
[.line-through]#.. Enter the required payload values with type `vCenter` and vCenter credentials.#
[.line-through]#.. Click *Execute*.#
[.line-through]#. (For NetApp HCI only) Add a compute node asset to the management node known assets:#
[.line-through]#.. Click *POST /assets/{asset_id}/compute-nodes* to add a compute node sub-asset with credentials for the compute node asset.#
[.line-through]#.. Click *Try it out*.#
[.line-through]#.. Enter the parent base asset ID you copied to your clipboard in the *asset_id* field.#
[.line-through]#.. In the payload, enter the required payload values as defined in the Model tab. Enter `ESXi Host` as `type` and paste the hardware tag you saved during a previous step for `hardware_tag`.#
[.line-through]#.. Click *Execute*.#
