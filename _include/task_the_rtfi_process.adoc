// This include file is used in the element-software and element-sfotware-123 repos. 
...
. Checks hardware.
. Tests hardware.
. Erases all selected drives securely.
...
For information on creating a USB key or using the BMC to perform the RTFI process, see link:task_rtfi_deployment_and_install_options.html[RTFI deployment and installation options]. 
...
+
WARNING: All data and configuration details are permanently erased from the node after the process is initiated. If you elect not to proceed, you are directed to the link:task_rtfi_options_menu.html[RTFI options menu].
+
