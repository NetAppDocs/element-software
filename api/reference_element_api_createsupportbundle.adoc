...
[.lead]
You can use the CreateSupportBundle to create a support bundle file under the node's directory. After creation, the bundle is stored on the node as a tar file (gz compression option is available via the extraArgs parameter.)
...
extraArgs
a|
This parameter is passed to '--compress gz' to create the support bundle as a tar.gz file.
a|
string
a|
None
