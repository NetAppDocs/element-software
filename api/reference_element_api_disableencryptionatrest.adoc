...
[NOTE]
====
* You cannot use this method to disable software encryption at rest. To disable software encryption at rest, you need to link:reference_element_api_createcluster.html[create a new cluster] with software encryption at rest disabled.

* To view the current status of encryption at rest, software encryption at rest, or both on the cluster, use the link:../api/reference_element_api_getclusterinfo.html[get cluster info method]. You can use the `GetSoftwareEncryptionAtRestInfo` link:../api/reference_element_api_getsoftwareencryptionatrestinfo.html[method to get information the cluster uses to encrypt data at rest].
====
