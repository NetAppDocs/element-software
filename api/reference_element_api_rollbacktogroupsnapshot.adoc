...
* Rolling back a volume to a snapshot or volumes to a group snapshot might fail when slice synchronization is in progress. Retry `RollbackToGroupSnapshot` after syncing completes.
====
