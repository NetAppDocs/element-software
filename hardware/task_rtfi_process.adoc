...
You can manually perform the RTFI process using a bootable USB key or the baseboard management controller (BMC). 

* To create a bootable USB key, see the Knowledge Base article https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/NetApp_HCI/How_to_create_an_RTFI_key_to_re-image_a_SolidFire_storage_node[How to create an RTFI key to re-image a SolidFire storage node^].
* For information on the RTFI process using the BMC, see the Knowledge Base article https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/NetApp_HCI/How_to_RTFI_an_HCI_Compute_Node_via_BMC[How to RTFI an HCI Compute Node using BMC^].
