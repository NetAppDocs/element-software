...
You can manually perform the RTFI process using a bootable USB key or the baseboard management controller (BMC). For information on using a USB key or the BMC, see link:task_rtfi_deployment_and_install_options.html[RTFI deployment and installation options]. 
