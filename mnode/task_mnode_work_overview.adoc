...
[.lead]
Element 12.5 brings improved remote access to individual nodes using link:../mnode/task_mnode_enable_node_troubleshooting_sessions.html[signed SSH certificates]. A new, limited-privilege local user account called `sfreadonly` enables access to the storage node backend for basic maintenance or troubleshooting purposes. You can also configure the `supportAdmin` access type for a cluster administrator user to allow NetApp support access the cluster on an as-needed basis
...
- link:task_mnode_enable_node_troubleshooting_sessions.html[Accessing storage nodes using SSH for basic troubleshooting]
* link:task_mnode_enable_remote_support_connections.html[Enable remote NetApp Support connections (Element 12.3 and earlier)]
