...
[.lead]
Element 12.5 brings improved remote access to individual nodes using signed SSH certificates. A new, limited-privilege local user account called `sfreadonly` enables access to the storage node backend for basic maintenance or troubleshooting purposes. You can also configure the `supportAdmin` access type for a cluster administrator user to allow NetApp support access the cluster on an as-needed basis. For more information; see link:../mnode/task_mnode_enable_node_troubleshooting_sessions.html[Accessing storage nodes using SSH for basic troubleshooting].
