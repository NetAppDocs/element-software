...
summary: For Element 12.5 and later, you can enable the sflocal user account on the management node to troubleshoot system components without help from NetApp Support. You can also enable and open remote support tunnel access for NetApp Support for advanced troubleshooting.
keywords: netapp, sflocal, connect to support, support tunnel, rst, mnode, management node
---
...
[.lead]
Beginning with Element 12.5, you can use the sflocal system account on the storage nodes for basic troubleshooting. You can also enable and open remote support tunnel access for NetApp Support for advanced troubleshooting.

The sflocal system account enables access to run basic Linux system and network troubleshooting commands, including `ipmitool` and `ping`.

WARNING: Unless advised by NetApp Support, any alterations to this system are unsupported, voiding your support contract, and may result in instability or inaccessibility of data.
...
== Troubleshoot a cluster node
You can perform basic troubleshooting using the sflocal system account:
...
. Run the script on the management node to generate the SSH keychain. The script enables SSH access using the sflocal system account across all nodes in the cluster.
+
----
...
+
----
ssh -i [cluster admin user name] sflocal@[node_ip]
----
+
...
+
----
ssh -i [cluster admin user name] sfadmin@[node_ip]
----
...
== Troubleshoot a node that is not part of cluster
You can perform basic troubleshooting of a node that has not yet been added to a cluster. You can use the sflocal system account for this purpose with or without the help of NetApp Support. If you have a management node set up, you can use it for SSH and run the script provided for this task.
...
+
----
ssh -i user [cluster admin user name] sflocal@[node_ip]
----
