...
You can configure `supportAdmin` access for cluster administrator accounts by using APIs and adding `"supportAdmin"` as the `"access"` type in the API request:
...
+
To get the `clusterAdminID` for a given account, you can use the link:../api/reference_element_api_listclusteradmins.html[ListClusterAdmins API].
