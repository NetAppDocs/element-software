...
. If there is a Dynamic Host Configuration Protocol (DHCP) on the network that assigns IPs with a maximum transmission unit (MTU) less than 1500 bytes, perform the following steps:
...
.. Using the terminal user interface (TUI), configure the correct IP on the management network with an MTU greater than or equal to 1500 bytes.
