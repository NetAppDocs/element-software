...
To manually install the management node for your cluster, you first download the appropriate ISO or OVA from the NetApp Support Site and install the VM.
...
After installing the management node, you create the management node admin user and configure the management node network. 
...
Before setting up the management node, you sync the time between the management node and the storage cluster.
...
To complete the installation configuration, locate the installation ID and add a vCenter controller asset.
...
== Step 6: Upgrade VM hardware version on the management node
After configuring the management node and upgrading to vSphere to 7.x or 8.x, you should also upgrade the VM hardware version on the management node.

.Steps
. Log in to the vSphere Web Client as a vCenter Administrator.
. From the vSphere Client Menu, select *VMs and Templates*.
. Right click on the cirtual machine, and select * *Compatibility* > *Upgrade VM Compatibility*.
. Select *Yes*.
