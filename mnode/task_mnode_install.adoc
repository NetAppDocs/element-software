...
After the VM installation completes, you create the management node admin user and configure the management node network. 
...
Element 12.8 includes vSphere to 7.x or 8. As a result, you must upgrade the VM hardware version on the management node to ESXi 6.7 or later before you upgrade the management node. This ensures that you have a supported ESXi version. 
