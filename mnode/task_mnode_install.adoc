...
[NOTE]
====
If there is a Dynamic Host Configuration Protocol (DHCP) server on the network that assigns IPs with a maximum transmission unit (MTU) less than 1500 bytes, you cannot configure the management node network or use the management node UI. To avoid this issue, you can use the following procedure:
