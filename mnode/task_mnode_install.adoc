...
[NOTE]
====
If there is a Dynamic Host Configuration Protocol (DHCP) server on the network that assigns IPs with a maximum transmission unit (MTU) less than 1500 bytes, you cannot configure the management node network or use the Element web UI. To avoid this issue, you can use the following procedure:
...
. Configure the correct IP on the management network with an MTU of greater than or equal to 1500 bytes.
