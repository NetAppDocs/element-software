...
After the installation completes, you create the management node admin user and configure the management node network. 
...
After setting up a management node and upgrading to vSphere to 7.x or 8.x, you should also upgrade the VM hardware version on the management node.
