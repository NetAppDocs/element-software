...
Before you begin, you should check if you have a Dynamic Host Configuration Protocol (DHCP) server on the network that assigns IPs with a maximum transmission unit (MTU) less than 1500 bytes, which prevents you configuring the management node network or using the management node UI. If you have a DHCP that assigns IPs with an MTU less than 1500 bytes, you should perform the following steps:
...
. Using the terminal user interface (TUI), configure the correct IP on the management network with an MTU of greater than or equal to 1500 bytes.
. Re-assign the correct VM network to the VM.

Create the management node admin and configure the network by performing the following steps. 

.Steps
. Using the TUI, create a management node admin user.
+
