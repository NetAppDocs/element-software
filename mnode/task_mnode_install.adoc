...
This manual process is intended for SolidFire all-flash storage administrators [.line-through]#and NetApp HCI administrators# who are not using the NetApp Deployment Engine for management node installation.
...
Prior to following this procedure, you should have an understanding of link:https://docs.netapp.com/us-en/hci/docs/concept_hci_volumes.html#persistent-volumes[Persistent volumes^] and whether or not you want to use them. Persistent volumes are optional but recommended for management node configuration data recovery in the event of a VM loss.
...
[.line-through]#. <<Configure compute node assets,(NetApp HCI only) Configure compute node assets>>#

== Download ISO or OVA and deploy the VM

. Download the OVA or ISO for your installation from link:https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab[Element software] on the NetApp Support Site.
[.line-through]#NetApp HCI: https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab#
...
. [.line-through]#(NetApp HCI only) Locate the hardware tag for your compute node in vSphere:
...
. Add a vCenter controller asset for [.line-through]#NetApp HCI monitoring (NetApp HCI installations only) and# Hybrid Cloud Control [.line-through]#(for all installations)# to the management node known assets:
...
[.line-through]#== (NetApp HCI only) Configure compute node assets#
...
[.line-through]#.. In the payload, enter the required payload values as defined in the Model tab. Enter `ESXi Host` as `type` and enter the hardware tag you saved during a previous step for `hardware_tag`.#
[.line-through]#.. Click *Execute*.#


[discrete]
== Find more Information
* link:https://docs.netapp.com/us-en/hci/docs/concept_hci_volumes.html#persistent-volumes[Persistent volumes^]
* link:task_mnode_add_assets.html[Add a control asset to the management node]
