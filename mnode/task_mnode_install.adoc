...
[NOTE]
====
Starting with Element 12.5, substeps (a) to (e) are performed automatically. For management node 12.5, proceed to substep (f) to complete the time sync configuration. As a result, when you run the link:task_mnode_install.html#set-up-the-management-node[setup command], you see output in the setup script similar to the following:
----
