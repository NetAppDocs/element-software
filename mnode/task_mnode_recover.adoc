...
[NOTE]
====
Starting with Element 12.5, substeps (a) to (e) are performed automatically. As a result, when you run the <<step_6_recover_mnode_redeploy,redeploy command>>, you see output in the redeploy script similar to the following:
----
...
----
Proceed to <<substep_f_recover_config_time_sync,substep (f)>> to complete the time sync configuration procedure.

It is not recommended to edit the NTP after you complete the time sync configuration because it affects the NTP when you run the redeploy command on the management node.
====
...
.. [[substep_f_recover_config_time_sync]]*Starting with Element 12.5, you can manually continue the procedure from here.*
+
...
. [[step_6_recover_mnode_redeploy]]Configure and run the management node redeploy command to connect to persistent volumes hosted on the cluster and start services with previous management node configuration data:
+
