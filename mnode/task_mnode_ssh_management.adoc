...
NOTE: The NetApp Hybrid cloud Control UI feature is not available at the read-only level.

.Steps
. In the configuration page, in the *Support Access for Management Node* tile.
. Toggle the *Management Node SSH Enable* to on.
. To disable the
