...
= Enable Active IQ and NetApp [.line-through]#HCI# monitoring
...
. If you have not already done so, add a vCenter controller asset for [.line-through]#NetApp HCI monitoring (NetApp HCI installations only) and# NetApp Hybrid Cloud Control to the management node known assets:
+
NOTE: A controller asset is required for NetApp [.line-through]#HCI# monitoring services.
