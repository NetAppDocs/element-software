...
All ports are TCP unless stated otherwise, and should permit bi-directional communications between the NetApp Support Server, management node, and nodes running Element software. The following example shows bidirectional communication for port 3260 between the iSCSI clients and the storage cluster SVIP: 
