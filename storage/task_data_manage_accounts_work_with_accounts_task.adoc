...
Beginning with Element 12.7, CHAP algorithms SHA1, SHA-256, and SHA3-256 are supported. After you upgrade to Element 12.7, iSCSI selects the most secure algorithm from the list requested by the host iSCSI initiator. If no SHA support is requested, the iSCSI target defaults back to MD5. You might need to change the host iSCSI initiator configuration to enable support for secure algorithms. If required, you can use the https://docs.netapp.com/us-en/element-software/api/reference_element_api_listiscsisessions.html[ListISCSISessions] API method to show the new algorithms being used. For more information, see the https://library.netapp.com/ecm/ecm_download_file/ECMLP2884468[Element Software 12.7 Release Notes^].
