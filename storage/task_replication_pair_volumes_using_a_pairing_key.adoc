...
[.lead]
If you have cluster administrator access to only the source cluster (you do not have cluster admin credentials for a remote cluster), you can pair a volume with another volume on a remote cluster using a pairing key.
