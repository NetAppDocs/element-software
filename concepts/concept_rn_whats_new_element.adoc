...
NOTE: The cumulative software and firmware updates are installed as part of an Element 12.8 or 12.7 upgrade based on the current Element version running on a storage cluster. For example, if a cluster is currently running Element 12.3.x, you can upgrade directly to Element 12.8 to get the cumulative updates from Element 12.8, 12.7, and 12.5. For supported upgrade paths, see this https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/Element_Software/What_is_the_upgrade_matrix_for_storage_clusters_running_NetApp_Element_software[KB article^]
...
The new Volume Load Balancing feature is introduced in Element 12.8. You can use this feature to balance volumes across nodes based on the actual IOPS by each volume instead of the minimum IOPS configured in the QoS policy. link:../storage/task_system_manage_cluster_volume_load_balancing.html[Learn more].

=== CHAP algorithm for iSCSI session

Beginning with Element 12.8, you can view the Challenge-Handshake Authentication Protocol (CHAP) for an iSCSI session. link:../storage/reference_monitor_iscsi_session_details.html[Learn more].
