...
== Manage volume accounts
link:link:concepts/concept_solidfire_concepts_accounts_and_permissions.html#volume-accounts[Volume accounts] are managed within the NetApp Hybrid Cloud Control Volumes table. These accounts are specific only to the storage cluster on which they were created. These types of accounts enable you to set permissions on volumes across the network, but have no effect outside of those volumes.
