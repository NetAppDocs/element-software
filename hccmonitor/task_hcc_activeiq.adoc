---
sidebar: sidebar
permalink: hccmonitor/task_hcc_activeiq.html
...
.. If you have more than one SolidFire all-flash storage installation, click *Select a Cluster* and choose the cluster from the list.
.. In the left navigation pane, click *Nodes*.
. If a node or nodes are missing from the list, contact NetApp Support.

TIP: To view the number of storage and compute resources, look at the Hybrid Cloud Control (HCC) Dashboard. See link:task_hcc_dashboard.html[Monitor storage resources with the HCC Dashboard].
